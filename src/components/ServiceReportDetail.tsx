...
const ServiceReportDetail = () => {
  ...
  const { data: evaluation, isLoading } = useQuery({
    queryKey: ['service-evaluation', id],
    queryFn: async () => {
  ...
  const { data: questions = [], isLoading: isLoadingDetails } = useQuery({
    ...
    queryFn: async () => {
      ...
      return answers?.map(answer => ({
        id: answer.service_questions?.id,
        question: answer.service_questions?.question || '',
        points: answer.service_questions?.points || 0,
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description: "The service evaluation has been successfully deleted.",
      });
  ...
  const handleExcelDownload = () => {
    ...
    const generalInfo = [
      ['CRS-Store Service Evaluation Report'],
      [],
    ...
    // Download file
    XLSX.writeFile(workbook, `Service_Report_${evaluation.store_name}_${format(new Date(evaluation.evaluation_date), 'dd-MM-yyyy')}.xlsx`);
  };
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-4 sm:mb-6">
          ...
          <div className="p-3 sm:p-4 rounded-lg bg-white border border-gray-200 shadow-sm">
            ...
            <p className="text-base sm:text-lg font-semibold text-red-600">{lostPoints}</p>
          </div>
          <div className="p-3 sm:p-4 rounded-lg bg-white border border-gray-200 shadow-sm">
