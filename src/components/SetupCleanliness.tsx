...
interface CleanlinessQuestion {
  id: number;
  question: string;
  points: number;
}

const SetupCleanliness = () => {
  const { toast } = useToast();
  const [localQuestions, setLocalQuestions] = useState<CleanlinessQuestion[]>([]);
  ...
  const saveQuestionsMutation = useMutation({
    mutationFn: async (questions: CleanlinessQuestion[]) => {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['cleanliness-questions'] });
  ...
  const updateQuestion = (id: number, field: keyof CleanlinessQuestion, value: string | number) => {
    ...
    <div className="p-4 sm:p-6 bg-gray-50 min-h-screen">
      <div className="w-full max-w-[calc(100vw-2rem)] sm:max-w-none overflow-hidden">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          <div>
            <h2 className="text-xl sm:text-2xl font-semibold mb-2 text-gray-900">Setup Cleanliness Form</h2>
...
export default SetupCleanliness;
